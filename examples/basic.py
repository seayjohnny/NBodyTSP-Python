...
def example_basic():
    ...
    simulator = TSPNBodySimulator("datasets/ch150/coords.txt")
...
def example_custom_params():
    """Example 2: Custom parameters for faster/different behavior."""
    print("\n" + "="*70)
    ...
    simulator = TSPNBodySimulator("datasets/bay29/coords.txt", custom_params)
...
def example_batch_processing():
    ...
    for dataset in datasets:
        ...
        results.append({
            ...
            'optimal_cost': simulator.optimal_cost * simulator.normalizing_factor if simulator.optimal_cost else None,
        })
    ...
    for result in results:
        ...
        if opt:
            ...
            print(f"{dataset_name:<30} {cities:<8} {cost:<12.2f} {opt:<12.2f} {pct_diff:+.2f}%")
        else:
            print(f"{dataset_name:<30} {cities:<8} {cost:<12.2f} {'N/A':<12} {'N/A':<8}")
...
def example_param_sweep():
        ...
        for force_cutoff in force_cutoff_values:
            for repulsion in repulsion_values:
                ...
                percent_diff = simulator.path_extractor.compare_with_optimal(
                    cost, simulator.optimal_cost * simulator.normalizing_factor if simulator.optimal_cost else None
                )['percent_difference']
                results.append((attraction, force_cutoff, repulsion, damping,cost, percent_diff))
                simulator.cleanup()
    ...
    print("="*70)
    print(f"{'Attraction':<20} {'Force Cutoff':<12} {'Repulsion':<12} {'Damping':<12} {'Cost':<12} {'% Diff':<8}")
    print("-"*70)
    for attraction, force_cutoff, repulsion, damping, cost, percent_diff in results:
        print(f"{attraction:<20} {force_cutoff:<12} {repulsion:<12} {damping:<12} {cost:<12.2f} {percent_diff:+.2f}%")
